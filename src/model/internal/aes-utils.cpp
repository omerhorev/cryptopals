//
// Created by omerh on 01/08/2019.
//

#include <cstring>
#include "model/internal/aes-utils.h"
#include "math/galois-field.h"

using namespace model;
using namespace model::internal;
using namespace math;

void aes_utils::generate_key_schedule(const unsigned char *root_key,
                                      size_t root_key_length,
                                      unsigned char *expanded_key,
                                      size_t expanded_key_length)
{
    for (int i = 0; i < root_key_length; ++i) expanded_key[i] = root_key[i];

    galois256 pow_index(0);
    galois256 two(2);
    auto write_index = root_key_length; // We start with 16 because we already copied 16 bytes of root key to the expanded

    while (write_index < expanded_key_length)
    {
        unsigned char t[4];

        for (int j = 0; j < 4; ++j)
        {
            t[j] = expanded_key[write_index + j - 4];
        }

        // If its the first 4 bytes in the 16 bytes block
        if (write_index % root_key_length == 0)
        {
            aes_utils::circular_rotate_left(t, sizeof(t), 1);

            for (unsigned char &j : t)
            {
                j = sbox_table[j];
            }

            t[0] ^= (unsigned char) (two ^ pow_index);
            pow_index++;
        }

        if (root_key_length == 32 && write_index % root_key_length == block_size)
        {
            for (unsigned char &i : t) i = sbox_table[i];
        }

        for (unsigned char k : t)
        {
            expanded_key[write_index] = expanded_key[write_index - root_key_length] ^ k;
            write_index++;
        }
    }
}

void aes_utils::circular_rotate_left(unsigned char *buffer, size_t length, unsigned int offset)
{
    offset = (unsigned int) (offset % length);

    auto temp = new unsigned char[length];

    for (int i = 0; i < length; ++i)
    {
        temp[i] = buffer[(i + offset) % length];
    }

    for (int i = 0; i < length; i++)
    {
        buffer[i] = temp[i];
    }

    delete[] temp;
}
void aes_utils::sub_bytes(aes_utils::state &s)
{
    for (auto &c : s.raw) c = sbox_table[c];
}

void aes_utils::sub_bytes_inv(state &s)
{
    for (auto &c : s.raw) c = sbox_inv_table[c];
}

void aes_utils::shift_rows(aes_utils::state &s)
{
    for (unsigned int r = 0; r < state_rows_count; ++r)
    {
        unsigned char row[4];
        for (int i = 0; i < 4; ++i) row[i] = s.matrix[i][r];

        circular_rotate_left(row, sizeof(row), r);

        for (int i = 0; i < 4; ++i) s.matrix[i][r] = row[i];
    }
}

void aes_utils::shift_rows_inv(state &state)
{
    for (unsigned int r = 0; r < state_rows_count; ++r)
    {
        unsigned char row[4];
        for (int i = 0; i < 4; ++i) row[i] = state.matrix[i][r];

        // Opposite of rotate left 1 is rotate left 3
        // Opposite of rotate left 2 is rotate left 2
        // Opposite of rotate left 3 is rotate left 1
        circular_rotate_left(row, sizeof(row), 4 - r);

        for (int i = 0; i < 4; ++i) state.matrix[i][r] = row[i];
    }
}

void aes_utils::mix_columns(aes_utils::state &s)
{
    for (auto &column : s.gmatrix)
    {
        math::galois256 a[4];

        for (auto c = 0; c < 4; c++)
        {
            a[c] = column[c];
        }

        //  __         __     __        __     __          __
        // |             |   |            |   |              |
        // |   col'[1]   |   |   col[1]   |   |  2  3  1  1  |
        // |   col'[2]   |   |   col[2]   |   |  1  2  3  1  |
        // |   col'[3]   | = |   col[3]   | X |  1  1  2  3  |
        // |   col'[4]   |   |   col[4]   |   |  3  1  1  2  |
        // |__         __|   |__        __|   |__          __|

        column[0] = (a[0] * 2) + (a[1] * 3) + (a[2] * 1) + (a[3] * 1);
        column[1] = (a[0] * 1) + (a[1] * 2) + (a[2] * 3) + (a[3] * 1);
        column[2] = (a[0] * 1) + (a[1] * 1) + (a[2] * 2) + (a[3] * 3);
        column[3] = (a[0] * 3) + (a[1] * 1) + (a[2] * 1) + (a[3] * 2);
    }
}



void aes_utils::mix_columns_inv(state &s)
{
    for (auto &column : s.gmatrix)
    {
        math::galois256 a[4];

        for (auto c = 0; c < 4; c++)
        {
            a[c] = column[c];
        }

        //  __        __     __         __     __              __
        // |            |   |             |   |                  |
        // |   col[1]   |   |   col'[1]   |   |  14  11  13  9   |
        // |   col[2]   |   |   col'[2]   |   |  9   14  11  13  |
        // |   col[3]   | = |   col'[3]   | X |  13  9   14  11  |
        // |   col[4]   |   |   col'[4]   |   |  11  13  9   14  |
        // |__        __|   |__         __|   |__              __|

        column[0] = (a[0] * 0x0E) + (a[1] * 0x0B) + (a[2] * 0x0D) + (a[3] * 0x09);
        column[1] = (a[0] * 0x09) + (a[1] * 0x0E) + (a[2] * 0x0B) + (a[3] * 0x0D);
        column[2] = (a[0] * 0x0D) + (a[1] * 0x09) + (a[2] * 0x0E) + (a[3] * 0x0B);
        column[3] = (a[0] * 0x0B) + (a[1] * 0x0D) + (a[2] * 0x09) + (a[3] * 0x0E);
    }
}


const unsigned char aes_utils::sbox_table[256] =
        {};

const unsigned char aes_utils::sbox_inv_table[256] =
        {,};

